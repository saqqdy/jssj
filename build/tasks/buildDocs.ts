    ...
    const builds = pkgs.map(async ({ name }) => {
        ...
        await runExecSync(
            `find ./ -type f -path "*.md" | xargs sed -i ${
                os === 'darwin' ? '""' : ''
            } "s/https:\u005C\u005Craw.githubusercontent.com\u005Csaqqdy\u005Cjssj/https:\u005C\u005Cgitee.com\u005Csaqqdy\u005Cjssj\u005Craw/g"`,
            RUN_PATH
        )
        // 生成静态文件
        ...
        await runExecSync(
            `find ./ -type f -path "*.md" | xargs sed -i ${
                os === 'darwin' ? '""' : ''
            } "s/https:\u005C\u005Cgitee.com\u005Csaqqdy\u005Cjssj\u005Craw/https:\u005C\u005Craw.githubusercontent.com\u005Csaqqdy\u005Cjssj/g"`,
            RUN_PATH
        )
    })
...
export async function deployDocs() {
    const builds = pkgs.map(async ({ name }) => {
        ...
        await runSpawnSync(
            `git push -f git@github.com:saqqdy/jssj.git master:gh-pages`,
            RUN_PATH
        )
        await runSpawnSync(
            `git push -f git@gitee.com:saqqdy/jssj.git master:gh-pages`,
            RUN_PATH
        )
    })
    await Promise.all(builds)
